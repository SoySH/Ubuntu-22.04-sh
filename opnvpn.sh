...
check_kernel() {
        ...
        if [[ $(uname -r | cut -d "." -f 1) -eq 2 ]]; then
                exiterr "The system is running an old kernel, which is incompati                                                                                                                                                                                                                                             ble with this installer."
        fi
}
...
check_os() {
        if grep -qs "ubuntu" /etc/os-release; then
                ...
                group_name="nogroup"
        elif [[ -e /etc/almalinux-release || -e /etc/rocky-release || -e /etc/ce                                                                                                                                                                                                                                             ntos-release ]]; then
                os="centos"
                os_version=$(grep -shoE '[0-9]+' /etc/almalinux-release /etc/roc                                                                                                                                                                                                                                             ky-release /etc/centos-release | head -1)
                group_name="nobody"
                ...
                exiterr "This installer seems to be running on an unsupported di                                                                                                                                                                                                                                             stribution.
Supported distros are Ubuntu, Debian, AlmaLinux, Rocky Linux, CentOS, Fedora, op                                                                                                                                                                                                                                             enSUSE and Amazon Linux 2."
        fi
}

check_os_ver() {
        if [[ "$os" == "ubuntu" && "$os_version" -lt 2004 ]]; then
                exiterr "Ubuntu 20.04 or higher is required to use this installe                                                                                                                                                                                                                                             r.
...
This version of Debian is too old and unsupported."
        ...
        if [[ "$os" == "centos" && "$os_version" -lt 8 ]]; then
                if ! grep -qs "Amazon Linux release 2 " /etc/system-release; the                                                                                                                                                                                                                                             n
                        exiterr "CentOS 8 or higher is required to use this inst                                                                                                                                                                                                                                             aller.
...
check_tun() {
        if [[ ! -e /dev/net/tun ]] || ! ( exec 7<>/dev/net/tun ) 2>/dev/null; th                                                                                                                                                                                                                                             en
...
set_client_name() {
        ...
        client=$(sed 's/[^0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQR                                                                                                                                                                                                                                             STUVWXYZ_-]/_/g' <<< "$unsanitized_client")
}
...
check_args() {
        if [ "$auto" != 0 ] && [ -e "$OVPN_CONF" ]; then
                show_usage "Invalid parameter '--auto'. OpenVPN is already set u                                                                                                                                                                                                                                             p on this server."
        fi
        if [ "$((add_client + export_client + list_clients + revoke_client))" -g                                                                                                                                                                                                                                             t 1 ]; then
        ...
        if [ "$remove_ovpn" = 1 ]; then
                if [ "$((add_client + export_client + list_clients + revoke_clie                                                                                                                                                                                                                                             nt + auto))" -gt 0 ]; then
        ...
        if [ "$((add_client + export_client + revoke_client))" = 1 ] && [ -n "$f                                                                                                                                                                                                                                             irst_client_name" ]; then
                show_usage "Invalid parameters. '--clientname' can only be speci                                                                                                                                                                                                                                             fied when installing OpenVPN."
        fi
        if [ -n "$listen_addr" ] || [ -n "$server_addr" ] || [ -n "$server_proto                                                                                                                                                                                                                                             " ] \
                || [ -n "$server_port" ] || [ -n "$first_client_name" ] || [ -n                                                                                                                                                                                                                                              "$dns1" ]; then
                        if [ -e "$OVPN_CONF" ]; then
                                show_usage "Invalid parameters. OpenVPN is alrea                                                                                                                                                                                                                                             dy set up on this server."
                        elif [ "$auto" = 0 ]; then
        ...
        if [ "$add_client" = 1 ]; then
                set_client_name
                if [ -z "$client" ]; then
                        exiterr "Invalid client name. Use one word only, no spec                                                                                                                                                                                                                                             ial characters except '-' and '_'."
        ...
        if [ "$export_client" = 1 ] || [ "$revoke_client" = 1 ]; then
                set_client_name
                if [ -z "$client" ] || [ ! -e /etc/openvpn/server/easy-rsa/pki/i                                                                                                                                                                                                                                             ssued/"$client".crt ]; then
        ...
        if [ -n "$listen_addr" ] && [ -z "$server_addr" ]; then
                show_usage "You must also specify the server address if the list                                                                                                                                                                                                                                             en address is specified."
        fi
        if [ -n "$server_addr" ] && { ! check_dns_name "$server_addr" && ! check                                                                                                                                                                                                                                             _ip "$server_addr"; }; then
                exiterr "Invalid server address. Must be a fully qualified domai                                                                                                                                                                                                                                             n name (FQDN) or an IPv4 address."
        fi
        ...
        if [ -z "$dns1" ] && [ -n "$dns2" ]; then
                show_usage "Invalid DNS server. --dns2 cannot be specified witho                                                                                                                                                                                                                                             ut --dns1."
        fi
        if [ -n "$dns1" ]; then
                dns=7
        else
...
check_nftables() {
        if [ "$os" = "centos" ]; then
                if grep -qs "hwdsl2 VPN script" /etc/sysconfig/nftables.conf \
                        || systemctl is-active --quiet nftables 2>/dev/null; the                                                                                                                                                                                                                                             n
...
install_wget() {
        # Detect some Debian minimal setups where neither wget nor curl are inst                                                                                                                                                                                                                                             alled
        if ! hash wget 2>/dev/null && ! hash curl 2>/dev/null; then
                if [ "$auto" = 0 ]; then
                        echo "Wget is required to use this installer."
                        read -n1 -r -p "Press any key to install Wget and contin                                                                                                                                                                                                                                             ue..."
                fi
...
install_iproute() {
        if ! hash ip 2>/dev/null; then
                if [ "$auto" = 0 ]; then
                        ...
                        read -n1 -r -p "Press any key to install iproute and con                                                                                                                                                                                                                                             tinue..."
                fi
...
OpenVPN Script
https://www.youtube.com/@slayerpsy
EOF
}
...
Bienvenido, instalar OpenVPN!
GitHub: https://github.com/SoySH
...
Install options (optional):
  ...
  --auto                         auto install OpenVPN using default or custom op                                                                                                                                                                                                                                             tions
  --listenaddr [IPv4 address]    IPv4 address that OpenVPN should listen on for                                                                                                                                                                                                                                              requests
  --serveraddr [DNS name or IP]  server address, must be a fully qualified domai                                                                                                                                                                                                                                             n name (FQDN) or an IPv4 address
  --proto [TCP or UDP]           protocol for OpenVPN (TCP or UDP, default: UDP)
  --port [number]                port for OpenVPN (1-65535, default: 1194)
  --clientname [client name]     name for the first OpenVPN client (default: cli                                                                                                                                                                                                                                             ent)
...
show_welcome() {
        if [ "$auto" = 0 ]; then
                ...
                echo 'You can use the default options and just press enter if yo                                                                                                                                                                                                                                             u are OK with them.'
        else
                show_header
                op_text=default
                if [ -n "$listen_addr" ] || [ -n "$server_addr" ] || [ -n "$serv                                                                                                                                                                                                                                             er_proto" ] \
                        || [ -n "$server_port" ] || [ -n "$first_client_name" ]                                                                                                                                                                                                                                              || [ -n "$dns1" ]; then
                        op_text=custom
                fi
                echo
...
enter_server_address() {
        echo
        echo "¿Quieres que los clientes de OpenVPN se conecten a este servidor u                                                                                                                                                                                                                                             sando un nombre DNS,"
        ...
        if [ "$use_dns_name" = 1 ]; then
                ...
                until check_dns_name "$server_addr_i"; do
                        ...
                        read -rp "Enter the DNS name of this VPN server: " serve                                                                                                                                                                                                                                             r_addr_i
                done
                detect_ip
...
find_public_ip() {
        ...
        get_public_ip=$(grep -m 1 -oE '^[0-9]{1,3}(\.[0-9]{1,3}){3}$' <<< "$(wge                                                                                                                                                                                                                                             t -T 10 -t 1 -4qO- "$ip_url1" || curl -m 10 -4Ls "$ip_url1")")
...
detect_ip() {
        ...
        if [[ $(ip -4 addr | grep inet | grep -vEc '127(\.[0-9]{1,3}){3}') -eq 1                                                                                                                                                                                                                                              ]]; then
                ...
                ip=$(ip -4 route get 1 | sed 's/ uid .*//' | awk '{print $NF;exi                                                                                                                                                                                                                                             t}' 2>/dev/null)
                if ! check_ip "$ip"; then
                        find_public_ip
                        ip_match=0
                        ...
                        if [ "$ip_match" = 0 ]; then
                                if [ "$auto" = 0 ]; then
                                        ...
                                        num_of_ip=$(ip -4 addr | grep inet | gre                                                                                                                                                                                                                                             p -vEc '127(\.[0-9]{1,3}){3}')
                                        ...
                                        until [[ -z "$ip_num" || "$ip_num" =~ ^[                                                                                                                                                                                                                                             0-9]+$ && "$ip_num" -le "$num_of_ip" ]]; do
                                                ...
                                                read -rp "IPv4 address [1]: " ip                                                                                                                                                                                                                                             _num
                                        done
                                        [[ -z "$ip_num" ]] && ip_num=1
                                else
                                        ip_num=1
...
check_nat_ip() {
        ...
        if check_pvt_ip "$ip"; then
                ...
                if ! check_ip "$get_public_ip"; then
                        if [ "$auto" = 0 ]; then
                                echo
                                echo "This server is behind NAT. What is the pub                                                                                                                                                                                                                                             lic IPv4 address?"
                                read -rp "Public IPv4 address: " public_ip
                                until check_ip "$public_ip"; do
                                        echo "Invalid input."
                                        read -rp "Public IPv4 address: " public_                                                                                                                                                                                                                                             ip
                                done
                        else
                                echo "Error: Could not detect this server's publ                                                                                                                                                                                                                                             ic IP." >&2
                                echo "Abort. No changes were made." >&2
                                exit 1
                        fi
                else
...
show_config() {
        if [ "$auto" != 0 ]; then
                ...
                [ -n "$server_port" ] && port_text="$server_port" || port_text=1                                                                                                                                                                                                                                             194
                [ -n "$first_client_name" ] && client_text="$client" || client_t                                                                                                                                                                                                                                             ext=client
...
detect_ipv6() {
        ...
        if [[ $(ip -6 addr | grep -c 'inet6 [23]') -ne 0 ]]; then
                ip6=$(ip -6 addr | grep 'inet6 [23]' | cut -d '/' -f 1 | grep -o                                                                                                                                                                                                                                             E '([0-9a-fA-F]{0,4}:){1,7}[0-9a-fA-F]{0,4}' | sed -n 1p)
        fi
}
...
select_protocol() {
        if [ "$auto" = 0 ]; then
                ...
                [ -n "$server_proto" ] && protocol="$server_proto" || protocol=u                                                                                                                                                                                                                                             dp
        fi
}
...
check_firewall() {
        ...
        if ! systemctl is-active --quiet firewalld.service && ! hash iptables 2>                                                                                                                                                                                                                                             /dev/null; then
                ...
                if [[ "$firewall" == "firewalld" ]]; then
                        ...
                        echo
                        echo "Note: firewalld, which is required to manage routi                                                                                                                                                                                                                                             ng tables, will also be installed."
                fi
        fi
}
...
disable_limitnproc() {
        ...
        if systemd-detect-virt -cq; then
                mkdir /etc/systemd/system/openvpn-server@server.service.d/ 2>/de                                                                                                                                                                                                                                             v/null
                echo "[Service]
LimitNPROC=infinity" > /etc/systemd/system/openvpn-server@server.service.d/disab                                                                                                                                                                                                                                             le-limitnproc.conf
        fi
}
...
install_pkgs() {
        if [[ "$os" = "debian" || "$os" = "ubuntu" ]]; then
                ...
                (
                        set -x
                        apt-get -yqq update || apt-get -yqq update
                        apt-get -yqq --no-install-recommends install openvpn >/d                                                                                                                                                                                                                                             ev/null
                ) || exiterr2
                (
                        set -x
                ...
                (
                        set -x
                        yum -y -q install openvpn openssl ca-certificates tar $f                                                                                                                                                                                                                                             irewall >/dev/null 2>&1
                ) || exiterr3
        elif [[ "$os" = "fedora" ]]; then
                (
                        set -x
                        dnf install -y openvpn openssl ca-certificates tar $fire                                                                                                                                                                                                                                             wall >/dev/null
                ) || exiterr "'dnf install' failed."
        else
                # Else, OS must be openSUSE
                (
                        set -x
                        zypper install -y openvpn openssl ca-certificates tar $f                                                                                                                                                                                                                                             irewall >/dev/null
                ) || exiterr4
        fi
...
create_firewall_rules() {
        if systemctl is-active --quiet firewalld.service; then
                # Using both permanent and not permanent rules to avoid a firewa                                                                                                                                                                                                                                             lld
                # reload.
                ...
                firewall-cmd -q --permanent --direct --add-rule ipv4 nat POSTROU                                                                                                                                                                                                                                             TING 0 -s 10.8.0.0/24 ! -d 10.8.0.0/24 -j MASQUERADE
                if [[ -n "$ip6" ]]; then
                        firewall-cmd -q --zone=trusted --add-source=fddd:1194:11                                                                                                                                                                                                                                             94:1194::/64
                        ...
                        firewall-cmd -q --permanent --direct --add-rule ipv6 nat                                                                                                                                                                                                                                              POSTROUTING 0 -s fddd:1194:1194:1194::/64 ! -d fddd:1194:1194:1194::/64 -j MASQ                                                                                                                                                                                                                                             UERADE
                fi
        else
                ...
                if [[ $(systemd-detect-virt) == "openvz" ]] && readlink -f "$(co                                                                                                                                                                                                                                             mmand -v iptables)" | grep -q "nft" && hash iptables-legacy 2>/dev/null; then
...
ExecStart=$iptables_path -I FORWARD -m state --state RELATED,ESTABLISHED -j ACCE                                                                                                                                                                                                                                             PT
ExecStop=$iptables_path -t nat -D POSTROUTING -s 10.8.0.0/24 ! -d 10.8.0.0/24 -j                                                                                                                                                                                                                                              MASQUERADE
ExecStop=$iptables_path -D INPUT -p $protocol --dport $port -j ACCEPT
ExecStop=$iptables_path -D FORWARD -s 10.8.0.0/24 -j ACCEPT
ExecStop=$iptables_path -D FORWARD -m state --state RELATED,ESTABLISHED -j ACCEP                                                                                                                                                                                                                                             T" > /etc/systemd/system/openvpn-iptables.service
                if [[ -n "$ip6" ]]; then
...
ExecStart=$ip6tables_path -I FORWARD -m state --state RELATED,ESTABLISHED -j ACC                                                                                                                                                                                                                                             EPT
ExecStop=$ip6tables_path -t nat -D POSTROUTING -s fddd:1194:1194:1194::/64 ! -d                                                                                                                                                                                                                                              fddd:1194:1194:1194::/64 -j MASQUERADE
ExecStop=$ip6tables_path -D FORWARD -s fddd:1194:1194:1194::/64 -j ACCEPT
ExecStop=$ip6tables_path -D FORWARD -m state --state RELATED,ESTABLISHED -j ACCE                                                                                                                                                                                                                                             PT" >> /etc/systemd/system/openvpn-iptables.service
                fi
...
WantedBy=multi-user.target" >> /etc/systemd/system/openvpn-iptables.service
                (
                        set -x
                        systemctl enable --now openvpn-iptables.service >/dev/nu                                                                                                                                                                                                                                             ll 2>&1
                )
        fi
}
...
remove_firewall_rules() {
        ...
        if systemctl is-active --quiet firewalld.service; then
                ip=$(firewall-cmd --direct --get-rules ipv4 nat POSTROUTING | gr                                                                                                                                                                                                                                             ep '\-s 10.8.0.0/24 '"'"'!'"'"' -d 10.8.0.0/24' | grep -oE '[^ ]+$')
                # Using both permanent and not permanent rules to avoid a firewa                                                                                                                                                                                                                                             lld reload.
                ...
                firewall-cmd -q --permanent --direct --remove-rule ipv4 nat POST                                                                                                                                                                                                                                             ROUTING 0 -s 10.8.0.0/24 ! -d 10.8.0.0/24 -j MASQUERADE
                if grep -qs "server-ipv6" "$OVPN_CONF"; then
                        ip6=$(firewall-cmd --direct --get-rules ipv6 nat POSTROU                                                                                                                                                                                                                                             TING | grep '\-s fddd:1194:1194:1194::/64 '"'"'!'"'"' -d fddd:1194:1194:1194::/6                                                                                                                                                                                                                                             4' | grep -oE '[^ ]+$')
                        firewall-cmd -q --zone=trusted --remove-source=fddd:1194                                                                                                                                                                                                                                             :1194:1194::/64
                        firewall-cmd -q --permanent --zone=trusted --remove-sour                                                                                                                                                                                                                                             ce=fddd:1194:1194:1194::/64
                        firewall-cmd -q --direct --remove-rule ipv6 nat POSTROUT                                                                                                                                                                                                                                             ING 0 -s fddd:1194:1194:1194::/64 ! -d fddd:1194:1194:1194::/64 -j MASQUERADE
                        firewall-cmd -q --permanent --direct --remove-rule ipv6                                                                                                                                                                                                                                              nat POSTROUTING 0 -s fddd:1194:1194:1194::/64 ! -d fddd:1194:1194:1194::/64 -j M                                                                                                                                                                                                                                             ASQUERADE
                fi
        else
...
install_easyrsa() {
        ...
        { wget -t 3 -T 30 -qO- "$easy_rsa_url" 2>/dev/null || curl -m 30 -sL "$e                                                                                                                                                                                                                                             asy_rsa_url" ; } | tar xz -C /etc/openvpn/server/easy-rsa/ --strip-components 1
...
create_pki_and_certs() {
        ...
        (
                set -x
                # Create the PKI, set up the CA and the server and client certif                                                                                                                                                                                                                                             icates
        ...
        cp pki/ca.crt pki/private/ca.key pki/issued/server.crt pki/private/serve                                                                                                                                                                                                                                             r.key pki/crl.pem /etc/openvpn/server
        # CRL is read with each client connection, while OpenVPN is dropped to n                                                                                                                                                                                                                                             obody
        chown nobody:"$group_name" /etc/openvpn/server/crl.pem
        # Without +x in the directory, OpenVPN can't run a stat() on the CRL fil                                                                                                                                                                                                                                             e
...
create_dns_config() {
        case "$dns" in
                1)
                        ...
                        grep -v '^#\|^;' "$resolv_conf" | grep '^nameserver' | g                                                                                                                                                                                                                                             rep -v '127.0.0.53' | grep -oE '[0-9]{1,3}(\.[0-9]{1,3}){3}' | while read line;                                                                                                                                                                                                                                              do
                                echo "push \"dhcp-option DNS $line\"" >> "$OVPN_                                                                                                                                                                                                                                             CONF"
                        done
                ;;
                2|"")
                ...
                ;;
                4)
                        echo 'push "dhcp-option DNS 208.67.222.222"' >> "$OVPN_C                                                                                                                                                                                                                                             ONF"
                        echo 'push "dhcp-option DNS 208.67.220.220"' >> "$OVPN_C                                                                                                                                                                                                                                             ONF"
                ;;
                5)
                        echo 'push "dhcp-option DNS 9.9.9.9"' >> "$OVPN_CONF"
                        echo 'push "dhcp-option DNS 149.112.112.112"' >> "$OVPN_                                                                                                                                                                                                                                             CONF"
                ;;
                6)
                        echo 'push "dhcp-option DNS 94.140.14.14"' >> "$OVPN_CON                                                                                                                                                                                                                                             F"
                        echo 'push "dhcp-option DNS 94.140.15.15"' >> "$OVPN_CON                                                                                                                                                                                                                                             F"
                ;;
                7)
                        echo "push \"dhcp-option DNS $dns1\"" >> "$OVPN_CONF"
                        if [ -n "$dns2" ]; then
                                echo "push \"dhcp-option DNS $dns2\"" >> "$OVPN_                                                                                                                                                                                                                                             CONF"
                        fi
                ;;
        esac
}
...
server 10.8.0.0 255.255.255.0" > "$OVPN_CONF"
    ...
    # Ajustes de MTU y MSS
    echo "tun-mtu 1400" >> "$OVPN_CONF"        # Ajuste del MTU en el servidor
    echo "mssfix 1360" >> "$OVPN_CONF"        # Ajuste del MSS para evitar fragm                                                                                                                                                                                                                                             entación

    # Otras configuraciones
    echo "keepalive 10 120" >> "$OVPN_CONF"
    echo "cipher AES-256-GCM" >> "$OVPN_CONF"
    echo "ncp-ciphers AES-128-GCM:AES-256-GCM" >> "$OVPN_CONF"
    echo "auth SHA384" >> "$OVPN_CONF"
    echo "user nobody" >> "$OVPN_CONF"
    echo "group $group_name" >> "$OVPN_CONF"
    echo "persist-key" >> "$OVPN_CONF"
    echo "persist-tun" >> "$OVPN_CONF"
    echo "verb 3" >> "$OVPN_CONF"
    echo "crl-verify crl.pem" >> "$OVPN_CONF"
...
get_export_dir() {
        ...
        export_dir=~/
        if [ -n "$SUDO_USER" ] && getent group "$SUDO_USER" >/dev/null 2>&1; the                                                                                                                                                                                                                                             n
...
new_client() {
        ...
        echo "</ca>"
        echo "<cert>"
        sed -ne '/BEGIN CERTIFICATE/,$ p' /etc/openvpn/server/easy-rsa/pki/issue                                                                                                                                                                                                                                             d/"$client".crt
        echo "</cert>"
        echo "<key>"
        ...
        if ! grep -qs "$ipt_cmd" /etc/rc.local; then
                if [ ! -f /etc/rc.local ]; then
                        ...
                        if [ "$os" = "ubuntu" ] || [ "$os" = "debian" ]; then
                                sed --follow-symlinks -i '/^exit 0/d' /etc/rc.lo                                                                                                                                                                                                                                             cal
                        fi
                fi
        ...
        if sestatus 2>/dev/null | grep "Current mode" | grep -q "enforcing" && [                                                                                                                                                                                                                                             [ "$port" != 1194 ]]; then
                ...
                if ! hash semanage 2>/dev/null; then
                        if [[ "$os_version" -eq 7 ]]; then
                                ...
                                (
                                        set -x
                                        dnf install -y policycoreutils-python-ut                                                                                                                                                                                                                                             ils >/dev/null
                                ) || exiterr "'dnf install' failed."
                        fi
                fi
...
create_client_common() {
    # Si el servidor está detrás de NAT, usar la IP correcta
    [[ -n "$public_ip" ]] && ip="$public_ip"

    # Crear client-common.txt como plantilla para agregar más usuarios
    echo "client
dev tun
proto $protocol
remote $ip $port
...
# Ajuste de MTU para el cliente
tun-mtu 1400            # Ajustamos el MTU del túnel para que sea 1400
mssfix 1360             # Ajustamos el MSS para evitar fragmentación de paquetes                                                                                                                                                                                                                                              TCP
ping 10
ping-restart 60
block-outside-dns
...
start_openvpn_service() {
        if [ "$os" != "openSUSE" ]; then
                (
                        set -x
                        systemctl enable --now openvpn-server@server.service >/d                                                                                                                                                                                                                                             ev/null 2>&1
                )
        else
...
select_menu_option() {
        ...
        echo "   1) Agregar un nuevo cliente"
        echo "   2) Exportar configuración para un cliente existente"
        echo "   3) Listar clientes existentes"
        echo "   4) Revocar un cliente existente"
        echo "   5) Eliminar OpenVPN"
        echo "   6) Salir"
...
enter_client_name() {
        ...
        set_client_name
        while [[ -z "$client" || -e /etc/openvpn/server/easy-rsa/pki/issued/"$cl                                                                                                                                                                                                                                             ient".crt ]]; do
                if [ -z "$client" ]; then
...
revoke_client_ovpn() {
        ...
        # CRL is read with each client connection, when OpenVPN is dropped to no                                                                                                                                                                                                                                             body
        chown nobody:"$group_name" /etc/openvpn/server/crl.pem
        remove_client_conf
}
...
disable_ovpn_service() {
        ...
        rm -f /etc/systemd/system/openvpn-server@server.service.d/disable-limitn                                                                                                                                                                                                                                             proc.conf
}

remove_sysctl_rules() {
        rm -f /etc/sysctl.d/99-openvpn-forward.conf /etc/sysctl.d/99-openvpn-opt                                                                                                                                                                                                                                             imize.conf
